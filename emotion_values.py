...
df_test = pd.read_csv('result_subject.csv', encoding='utf-8')
df_test['sentiment_value'] = 0  # 增加测试集sentiment_value列，并初始化为0
df_test['sentiment_word'] = ''  # 增加测试集sentiment_word列，并设置空值
# df_train.info()   # 查看训练集
# df_test.info()    # 查看测试集
# df_test.head()    # 查看测试集前几行

#
selected_features = ['content', 'sentiment_value']
x_train = df_train[selected_features]
x_test = df_test[selected_features]
y_train = df_train['sentiment_value']


# x_train['sentiment_value'].value_counts()   # 查看训练集各主题数量
# x_test['sentiment_value'].value_counts()    # 查看测试集主题数量，当前主题为空值
...
pipe_submission = pd.DataFrame({'content_id':df_test['content_id'], 'subject':df_test['subject'], 'sentiment_value':y_pred,
                                 'sentiment_word':df_test['sentiment_word']})
pipe_submission.to_csv('result_emotion.csv', index=False)

# 计时结束
t_end = time.time()
print("已将情感值训练完成，共耗时：{}min".format((t_end - t_start) / 60))
